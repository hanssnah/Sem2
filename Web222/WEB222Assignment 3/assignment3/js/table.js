...
function userLoad(){
    var user = document.body.querySelector("#thirdtable");
    var usertable= "<table border='1'>";
    for(var i=0; i< users.length; i++){
        if(i===0){
            usertable += "<tr>" + "<th> First Name </th>";
            usertable += "<th> Last Name </th>";
            usertable += "<th> Age </th>";
            usertable += "<th> Email </th>"+ "</tr>";
            
        }
        else{
            usertable += "<tr>" + "<td>" + users[i].first_name + "</td>";
            usertable += "<td>"+ users[i].last_name+  "</td>";
            usertable += "<td>" + users[i].age+ "</td>";
            usertable += "<td><a href = 'mailto:"+users[i].email+"'>"+ users[i].email+ "</td>"+ "</tr>";
            
        }

    }	

    user.innerHTML += usertable;
